...
def show_credentials():
    ...
    ongoing = Event.objects.filter(status__in=['published', 'registration_closed', 'in_progress',
                                   'scoring_closed', 'results_published']).exclude(result__published=True).distinct().count()
    upcoming = Event.objects.filter(status='draft').count()
