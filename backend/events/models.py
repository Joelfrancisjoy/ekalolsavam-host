...
class Event(models.Model):
    ...
    volunteers = models.ManyToManyField(
        User, related_name='assigned_volunteer_events', blank=True)
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='created_events')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    status = models.CharField(
        max_length=30, choices=EVENT_STATUS, default='draft')
...
class Judge(models.Model):
    ...
    assigned_events = models.ManyToManyField(
        Event, related_name='assigned_judges', blank=True)
...
class ParticipantVerification(models.Model):
    ...
    volunteer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='verified_participants')
