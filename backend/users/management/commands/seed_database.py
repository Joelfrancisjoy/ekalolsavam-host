...
class Command(BaseCommand):
    ...
    def seed_venues(self):
        ...
        if not self.venues:
            self.stdout.write(self.style.WARNING('No venues found. Run: python manage.py seed_venues'))
        else:
    ...
    def seed_users(self):
        ...
        # Admin users
        admin_users = [
            {'username': 'admin', 'email': 'admin@kalolsavam.edu', 'first_name': 'System', 'last_name': 'Admin'},
            {'username': 'coordinator', 'email': 'coordinator@kalolsavam.edu', 'first_name': 'Event', 'last_name': 'Coordinator'},
        ]
        ...
        # Judge users
        judge_data = [
            {'username': 'judge_music', 'email': 'judge.music@kalolsavam.edu', 'first_name': 'Ramesh', 'last_name': 'Kumar', 'spec': 'Music'},
            {'username': 'judge_dance', 'email': 'judge.dance@kalolsavam.edu', 'first_name': 'Priya', 'last_name': 'Nair', 'spec': 'Dance'},
            {'username': 'judge_theatre', 'email': 'judge.theatre@kalolsavam.edu', 'first_name': 'Suresh', 'last_name': 'Menon', 'spec': 'Theatre'},
            {'username': 'judge_literary', 'email': 'judge.literary@kalolsavam.edu', 'first_name': 'Lakshmi', 'last_name': 'Pillai', 'spec': 'Literary'},
            {'username': 'judge_arts', 'email': 'judge.arts@kalolsavam.edu', 'first_name': 'Vinod', 'last_name': 'Krishnan', 'spec': 'Visual Arts'},
            {'username': 'judge_music2', 'email': 'judge.music2@kalolsavam.edu', 'first_name': 'Maya', 'last_name': 'Sharma', 'spec': 'Music'},
        ]
        ...
        for jdata in judge_data:
            ...
            if created:
                ...
                Judge.objects.get_or_create(user=user, defaults={'specialization': spec})
                self.credentials.append({
                    'role': 'Judge',
                    'username': jdata['username'],
                    'password': 'judge123',
        ...
        # Volunteer users
        volunteer_data = [
            {'username': 'volunteer1', 'email': 'volunteer1@kalolsavam.edu', 'first_name': 'Arun', 'last_name': 'Das'},
            {'username': 'volunteer2', 'email': 'volunteer2@kalolsavam.edu', 'first_name': 'Sneha', 'last_name': 'Raj'},
            {'username': 'volunteer3', 'email': 'volunteer3@kalolsavam.edu', 'first_name': 'Kiran', 'last_name': 'Thomas'},
            {'username': 'volunteer4', 'email': 'volunteer4@kalolsavam.edu', 'first_name': 'Anjali', 'last_name': 'Varma'},
        ]
        ...
        last_names = ['Kumar', 'Nair', 'Menon', 'Pillai', 'Sharma', 'Varma', 'Das', 'Krishnan', 'Raj', 'Thomas']
        ...
        self.stdout.write(f'  Created {sum(len(v) for v in self.users_created.values())} users')
    ...
    def seed_events(self):
        ...
        if not self.venues:
            self.stdout.write(self.style.WARNING('No venues available, skipping events'))
            return

        admin = self.users_created['admin'][0] if self.users_created['admin'] else User.objects.filter(role='admin').first()
        
        # Event templates
        event_templates = [
            # Completed events (with results)
            {'name': 'Classical Dance Competition', 'category': 'dance', 'days_ago': 15, 'status': 'completed'},
            {'name': 'Group Song', 'category': 'music', 'days_ago': 14, 'status': 'completed'},
            {'name': 'Mono Act', 'category': 'theatre', 'days_ago': 13, 'status': 'completed'},
            {'name': 'Painting Competition', 'category': 'visual_arts', 'days_ago': 12, 'status': 'completed'},
            {'name': 'Essay Writing', 'category': 'literary', 'days_ago': 11, 'status': 'completed'},
            
            # Ongoing events (scores being entered)
            {'name': 'Folk Dance', 'category': 'dance', 'days_ago': 1, 'status': 'ongoing'},
            {'name': 'Solo Singing', 'category': 'music', 'days_ago': 0, 'status': 'ongoing'},
            {'name': 'Mime', 'category': 'theatre', 'days_ago': 0, 'status': 'ongoing'},
            
            # Upcoming events
            {'name': 'Western Dance', 'category': 'dance', 'days_ago': -2, 'status': 'upcoming'},
            {'name': 'Instrumental Music', 'category': 'music', 'days_ago': -3, 'status': 'upcoming'},
            {'name': 'Skit', 'category': 'theatre', 'days_ago': -4, 'status': 'upcoming'},
            {'name': 'Pencil Drawing', 'category': 'visual_arts', 'days_ago': -5, 'status': 'upcoming'},
            {'name': 'Poetry Recitation', 'category': 'literary', 'days_ago': -6, 'status': 'upcoming'},
        ]
        ...
        for template in event_templates:
            ...
            selected_judges = random.sample(available_judges, min(2, len(available_judges)))
            ...
            event, created = Event.objects.get_or_create(
                ...
                defaults={
                    ...
                    'max_participants': 30,
                    'created_by': admin,
                    'is_published': template['status'] != 'upcoming',
                }
            )
            ...
            if created:
                ...
                self.events.append({'event': event, 'status': template['status']})
                self.stdout.write(f'  Created event: {template["name"]} ({template["status"]})')
    ...
    def seed_registrations(self):
        ...
        self.stdout.write(f'  Created registrations for {len(self.events)} events')
    ...
    def seed_scores(self):
        ...
        for event_data in self.events:
            ...
            for reg in registrations:
                ...
                judges_to_score = judges if status == 'completed' else random.sample(judges, random.randint(1, len(judges)))
    ...
    def seed_results(self):
        ...
        for event_data in self.events:
            ...
            for reg in registrations:
                scores = Score.objects.filter(event=event, participant=reg.participant)
                if scores.exists():
    ...
    def print_credentials(self):
        ...
        for cred in self.credentials:
            ...
            if 'specialization' in cred:
                self.stdout.write(f"  Specialization: {cred['specialization']}")
            if 'school' in cred:
        ...
        self.stdout.write(f"      Only first 5 student credentials shown above for brevity")
        ...
        self.stdout.write(f"Volunteers: {len(self.users_created['volunteer'])}")
        self.stdout.write(f"School Accounts: {len(self.users_created['school'])}")
        self.stdout.write(f"Students: {len(self.users_created['student'])}")
        self.stdout.write(f"Events: {len(self.events)}")
        self.stdout.write(f"  - Completed (with results): {len([e for e in self.events if e['status'] == 'completed'])}")
        self.stdout.write(f"  - Ongoing (being scored): {len([e for e in self.events if e['status'] == 'ongoing'])}")
        self.stdout.write(f"  - Upcoming: {len([e for e in self.events if e['status'] == 'upcoming'])}")
        self.stdout.write('='*80 + '\n')
